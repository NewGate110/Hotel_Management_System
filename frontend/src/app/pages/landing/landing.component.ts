...
@Component({
  ...
  template: `
    <!-- ─── HERO ─── -->
    <section style="position: relative; min-height: 92vh; display: flex; flex-direction: column; justify-content: flex-end; overflow: hidden; padding-bottom: 48px;">
      <img
      ...
      <div class="container-wide" style="position: relative; z-index: 10; padding-top: 140px;">
        <p class="eyebrow" style="color: var(--clay-200); letter-spacing: var(--ls-widest);">Grand Plaza Hotel</p>

        <h1 style="font-family: var(--font-display); font-size: clamp(44px,7vw,84px); font-weight: 300; letter-spacing: -0.035em; line-height: 1.02; color: var(--sand-50); max-width: 960px; margin: 16px 0 0; text-wrap: balance;">
        ...
        <!-- Booking widget -->
        <div [formGroup]="searchForm" style="margin-top: 32px; display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 16px; align-items: flex-start; background: var(--surface); border-radius: var(--r-lg); padding: 20px 24px; max-width: 860px; box-shadow: var(--shadow-lg);" class="booking-widget">
          <!-- Check-in -->
        ...
        @if (dateError()) {
          <div style="margin-top: 16px; padding: 12px 14px; background: var(--clay-100); border-radius: var(--r-md); font-size: var(--fs-sm); color: var(--clay-700);">
            {{ dateError() }}
          </div>
        }

        @if (!searched()) {
          <p style="margin-top: 32px; display: flex; align-items: center; gap: 6px; font-size: var(--fs-xs); color: rgba(250,247,242,0.4);">
            <span class="material-icons-outlined" style="font-size: 16px;" aria-hidden="true">south</span>
            Scroll to explore
          </p>
        }
      </div>
      ...
      <div class="container-wide">
        <div style="display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 24px;">
          <div>
            ...
            <h2 style="font-family: var(--font-display); font-size: var(--fs-4xl); font-weight: 400; letter-spacing: var(--ls-tight); color: var(--fg); margin: 12px 0 0; text-wrap: balance; max-width: 560px;">
              Explore our exclusive collection of rooms &amp; villas
            </h2>
          </div>
          <a routerLink="/rooms/search" class="btn btn-secondary" style="flex-shrink: 0;">View all rooms →</a>
        </div>
    ...
    <!-- ─── AMENITIES STRIP ─── -->
    <section style="padding: 48px 0; background: var(--surface); border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);">
      <div class="container-wide" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 48px 64px;">
        @for (amenity of amenities; track amenity.label) {
          <div style="display: flex; align-items: center; gap: 10px; font-size: var(--fs-sm); font-weight: 500; color: var(--fg-2);">
            <span class="material-icons-outlined" style="font-size: 22px; color: var(--brand);" aria-hidden="true">{{ amenity.icon }}</span>
            {{ amenity.label }}
          </div>
        }
      </div>
        ...
        <div style="margin-top: 40px; display: grid; grid-template-columns: 1fr 1fr; gap: 16px; grid-template-rows: auto auto;" class="gallery-grid">
          <a routerLink="/rooms/search" class="gallery-card" style="grid-row: span 2; position: relative; overflow: hidden; border-radius: var(--r-xl); height: 456px; display: block; text-decoration: none;">
            ...
            <div style="position: absolute; bottom: 0; left: 0; padding: 28px;">
              ...
              <p style="margin-top: 4px; font-family: var(--font-display); font-size: var(--fs-2xl); font-weight: 400; color: var(--sand-50);">Overwater Bungalow</p>
              <p style="margin-top: 6px; font-size: 13px; color: rgba(250,247,242,0.6);">Direct lagoon access · Private sun deck · Glass floor</p>
              <span style="margin-top: 16px; display: inline-block; font-size: 13px; font-weight: 500; color: var(--clay-200); text-decoration: underline; text-underline-offset: 3px;">Explore rooms →</span>
            </div>
          </a>
          <a routerLink="/rooms/search" class="gallery-card" style="position: relative; height: 220px; overflow: hidden; border-radius: var(--r-xl); display: block; text-decoration: none;">
            <img src="https://images.unsplash.com/photo-1573843981267-be1999ff37cd?auto=format&fit=crop&w=900&q=80" alt="Beach Villa" style="width: 100%; height: 100%; object-fit: cover; transition: transform var(--dur-glide) var(--ease-glide);" class="gallery-img" />
            <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(8,24,32,0.75) 0%, transparent 60%);"></div>
            <div style="position: absolute; bottom: 0; left: 0; padding: 20px;">
              <p style="font-family: var(--font-display); font-size: var(--fs-lg); color: var(--sand-50);">Beach Villa</p>
              <p style="margin-top: 4px; font-size: 13px; color: rgba(250,247,242,0.6);">Private beach · Ocean breeze</p>
            </div>
          </a>
          <a routerLink="/rooms/search" class="gallery-card" style="position: relative; height: 220px; overflow: hidden; border-radius: var(--r-xl); display: block; text-decoration: none;">
            <img src="https://images.unsplash.com/photo-1583212292454-1fe6229603b7?auto=format&fit=crop&w=900&q=80" alt="Ocean Suite" style="width: 100%; height: 100%; object-fit: cover; transition: transform var(--dur-glide) var(--ease-glide);" class="gallery-img" />
            <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(8,24,32,0.75) 0%, transparent 60%);"></div>
            <div style="position: absolute; bottom: 0; left: 0; padding: 20px;">
              <p style="font-family: var(--font-display); font-size: var(--fs-lg); color: var(--sand-50);">Ocean Suite</p>
              <p style="margin-top: 4px; font-size: 13px; color: rgba(250,247,242,0.6);">360° panoramic views · Infinity pool</p>
            </div>
          </a>
        </div>
      </div>
    ...
    <section style="padding: 128px 0; background: var(--azure-800);">
      <div class="container">
        ...
        <div style="max-width: 720px; margin-top: 48px;">
          <blockquote style="font-family: var(--font-display); font-style: italic; font-weight: 300; font-size: 40px; letter-spacing: -0.02em; line-height: 1.25; color: var(--sand-50); margin: 0;">
            "{{ reviews[activeReview()].text }}"
          </blockquote>
          <div style="margin-top: 28px; padding-top: 24px; border-top: 1px solid rgba(250,247,242,0.2); display: flex; align-items: center; justify-content: space-between;">
            <div>
              <p style="font-size: var(--fs-base); font-weight: 500; color: var(--sand-50);">{{ reviews[activeReview()].name }}</p>
              <p style="font-size: var(--fs-sm); color: var(--sand-300); margin-top: 2px;">{{ reviews[activeReview()].role }}</p>
            </div>
            <div style="display: flex; gap: 8px;">
              <button type="button" class="btn btn-ghost btn-icon" style="color: var(--sand-50); border: 1px solid rgba(250,247,242,0.3);" (click)="prevReview()">
                <span class="material-icons-outlined" style="font-size: 18px;">arrow_back</span>
              </button>
              <button type="button" class="btn btn-ghost btn-icon" style="color: var(--sand-50); border: 1px solid rgba(250,247,242,0.3);" (click)="nextReview()">
    ...
    <!-- ─── FAQ ─── -->
    <section style="padding: 128px 0; background: var(--bg-alt);">
      <div class="container-wide" style="display: grid; grid-template-columns: 1fr 1fr; gap: 64px; align-items: start;" class="faq-grid">
        <div>
          <p class="eyebrow">FAQs</p>
          <h2 style="font-family: var(--font-display); font-size: var(--fs-4xl); font-weight: 400; letter-spacing: var(--ls-tight); color: var(--fg); margin: 12px 0 0; text-wrap: balance;">
            Frequently Asked Questions
          </h2>
          <p style="margin-top: 20px; font-size: var(--fs-base); line-height: var(--lh-normal); color: var(--fg-2);">
        ...
        <div>
          @for (faq of faqs; track faq.q; let i = $index) {
            <div style="border-bottom: 1px solid var(--border);">
              ...
              @if (openFaq() === i) {
                <p style="font-size: var(--fs-sm); line-height: var(--lh-loose); color: var(--fg-2); padding-bottom: 20px; margin: 0;">{{ faq.a }}</p>
              }
            </div>
          }
        </div>
    ...
    <section style="position: relative; overflow: hidden; padding: 112px 0;">
      <img
        ...
        aria-hidden="true"
      />
      <div style="pointer-events: none; position: absolute; inset: 0; background: rgba(8,24,32,0.7);" aria-hidden="true"></div>
      <div class="container" style="position: relative; z-index: 10; text-align: center;">
        <h2 style="font-family: var(--font-display); font-size: clamp(32px, 5vw, 56px); font-weight: 300; letter-spacing: var(--ls-tight); color: var(--sand-50); margin: 0; text-wrap: balance;">
          Don't wait any longer.<br />Start your adventure today.
        </h2>
        <p style="margin-top: 20px; font-size: var(--fs-base); line-height: var(--lh-normal); color: rgba(250,247,242,0.6); max-width: 500px; margin-left: auto; margin-right: auto;">
        ...
        <div style="margin-top: 32px; display: flex; flex-wrap: wrap; justify-content: center; gap: 12px;">
          ...
          <a routerLink="/contact" class="btn" style="color: var(--sand-50); background: rgba(250,247,242,0.1); border-color: rgba(250,247,242,0.25); font-size: 15px; padding: 16px 28px;">
            Contact Us
          </a>
        </div>
      </div>
      ...
      <div class="container-wide">
        <div style="display: grid; grid-template-columns: 1.5fr 1fr 1fr 1fr; gap: 48px; padding-bottom: 64px; border-bottom: 1px solid rgba(212,200,179,0.15);" class="footer-grid">
          <div>
            ...
            <p style="margin-top: 24px; font-size: var(--fs-sm); color: var(--sand-300); line-height: var(--lh-normal); max-width: 320px;">
              Personalised luxury from your own private island in the heart of the Indian Ocean.
            </p>
            <div style="margin-top: 20px; display: flex; gap: 16px;">
              <span class="material-icons-outlined" style="cursor: pointer; font-size: 20px; color: var(--sand-500); transition: color var(--dur-fast) var(--ease-out);" onmouseenter="this.style.color='var(--sand-200)'" onmouseleave="this.style.color='var(--sand-500)'">language</span>
              <span class="material-icons-outlined" style="cursor: pointer; font-size: 20px; color: var(--sand-500); transition: color var(--dur-fast) var(--ease-out);" onmouseenter="this.style.color='var(--sand-200)'" onmouseleave="this.style.color='var(--sand-500)'">phone</span>
              <span class="material-icons-outlined" style="cursor: pointer; font-size: 20px; color: var(--sand-500); transition: color var(--dur-fast) var(--ease-out);" onmouseenter="this.style.color='var(--sand-200)'" onmouseleave="this.style.color='var(--sand-500)'">mail</span>
            </div>
          </div>

          @for (col of footerCols; track col.title) {
            <div>
              <p style="font-size: 11px; font-weight: 500; letter-spacing: 0.14em; text-transform: uppercase; color: var(--sand-400); margin: 0 0 16px;">{{ col.title }}</p>
        ...
        <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; padding: 24px 0; font-size: var(--fs-xs); color: var(--sand-400);">
          <p style="margin: 0;">© {{ year }} Grand Plaza. All rights reserved.</p>
          <div style="display: flex; gap: 24px;">
            <span style="cursor: pointer; transition: color var(--dur-fast);" onmouseenter="this.style.color='var(--sand-200)'" onmouseleave="this.style.color='var(--sand-400)'">Privacy Policy</span>
            <span style="cursor: pointer; transition: color var(--dur-fast);" onmouseenter="this.style.color='var(--sand-200)'" onmouseleave="this.style.color='var(--sand-400)'">Terms &amp; Conditions</span>
          </div>
        </div>
      </div>
    </footer>
    ...
    <style>
      @media (max-width: 768px) {
        ...
        .suites-grid    { grid-template-columns: 1fr !important; }
        .experiences-grid { grid-template-columns: 1fr 1fr !important; }
        .gallery-grid   { grid-template-columns: 1fr !important; }
        .faq-grid       { grid-template-columns: 1fr !important; }
        .footer-grid    { grid-template-columns: 1fr 1fr !important; }
        .results-grid   { grid-template-columns: 1fr !important; }
      }
      .suite-card:hover { box-shadow: var(--shadow-md) !important; }
      .suite-card:hover .suite-img { transform: scale(1.03); }
...
export class LandingComponent implements OnInit {
  ...
  search(): void {
    ...
    if (!checkIn) { this.dateError.set('Please select a check-in date.'); return; }
    if (!checkOut) { this.dateError.set('Please select a check-out date.'); return; }
    if (checkOut <= checkIn) { this.dateError.set('Check-out date must be after check-in date.'); return; }
    ...
    setTimeout(() => document.getElementById('search-results')?.scrollIntoView({ behavior: 'smooth' }), 0);

    this.roomsApi.searchRooms({ checkIn: checkInStr, checkOut: checkOutStr, guests: guestCount }).subscribe({
  ...
  readonly experiences = [
    ...
    { icon: 'restaurant', label: 'Fine Dining', blurb: 'Candlelit dinners above the lagoon. Locally sourced, seasonally led.' },
    { icon: 'waves', label: 'Water Sports', blurb: 'Snorkel, kayak, and dive from your private beach at any hour.' },
    { icon: 'wb_sunny', label: 'Golden Hour', blurb: 'Guided sunset cruises and rooftop cocktails every evening.' },
  ];
  ...
  readonly faqs: Faq[] = [
    { q: 'How do I book a room at Grand Plaza?', a: 'Browse rooms directly on the website, select your preferred villa type and travel dates, then complete your reservation with your guest account. Instant confirmation is sent by email.' },
    { q: 'What is included in the room rate?', a: 'All rates include daily breakfast for two, complimentary snorkelling equipment, high-speed Wi-Fi, and non-motorised water sports. Premium packages add full-board dining and spa credits.' },
    { q: 'How do I reach the hotel?', a: 'Grand Plaza is a 30-minute speedboat transfer or a 15-minute scenic seaplane flight from the international airport. Private transfers can be arranged at booking.' },
    { q: 'What is the cancellation policy?', a: 'Bookings cancelled 14 or more days before check-in receive a full refund. Cancellations within 14 days incur a one-night retention fee. No-shows are non-refundable.' },
    { q: 'Do you host private events?', a: 'Yes — we specialise in intimate beach ceremonies, sunset lagoon dinners, and corporate retreats. Contact our events concierge team for a bespoke proposal.' },
  ];

  readonly reviews: Review[] = [
    { name: 'Sophia Linden', role: 'Honeymooner · United Kingdom', text: 'An absolute dream from start to finish. The overwater bungalow was breathtaking, and the staff anticipated our every need before we even thought to ask.', initials: 'SL' },
    { name: 'James Whitfield', role: 'Business Traveller · Australia', text: 'Impeccable service paired with jaw-dropping surroundings. The glass-floor bungalow and private sun deck made this the most luxurious trip I have ever taken.', initials: 'JW' },
    { name: 'Aisha Rahman', role: 'Family Holiday · UAE', text: 'The family villa was perfect for all of us. Our children loved the marine biologist snorkel tour — they have not stopped talking about it since we got home.', initials: 'AR' },
    { name: 'Marcus Chen', role: 'Solo Traveller · Singapore', text: 'I booked for a week and wanted to stay indefinitely. The sunset deck dinners, the spa, and the quiet of the private island are beyond anything I have experienced.', initials: 'MC' },
  ];
  ...
  readonly footerCols = [
    ...
    { title: 'Guest Services', links: [
      { label: 'Sign In', path: '/login' },
      { label: 'Register', path: '/register' },
      { label: 'Book a Room', path: '/rooms/search' },
      { label: 'Support', path: '/contact' },
    ]},
    { title: 'On Property', links: [
